	...
	def parse(self):
		...
		for div in theaters:
			...
			for div_movie in movies:
				...
				times = times.findAll('span')
				for div_time in times:
					if len(div_time.contents) > 1:
						time_val = div_time.contents[len(div_time.contents) - 1]
						try:
							...
							resp['theater'][index]['movies'][index_m]['times'].append("missing time")
		return resp
